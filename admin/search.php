...
echo "<table width='400'><thead><th>Select</th><th>Name</th><th>ID</th><th>Attending</th></thead>";
if (!$query->rowCount() == 0) {
    while ($results = $query->fetch()) {
        echo "<tr class='normal' id='hidden'><td><input type='checkbox' name='select' value='checked'/></td><td>", $results['name'], "</td><td>", $results['ID'],"</td><td>", $results['attending'], "</td></tr>";
    }
} else {
    echo 'Nothing found';
}
